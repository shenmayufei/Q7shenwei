...
import com.spd.qsevendemo.BuildConfig;
...
public final class Logcat {
    ...
    private static void log(final int type, final String tag, final Object... contents) {
        boolean tempIf = !sLog2ConsoleSwitch && !sLog2FileSwitch;
        if (!sLogSwitch || tempIf) {
            return;
        }
        ...
        boolean tempIf1 = (sLog2FileSwitch || typeHigh == FILE);
        if ( tempIf1&& typeLow >= sFileFilter) {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = AppSeven.getInstance().getPackageManager()
                    .getPackageInfo(AppSeven.getInstance().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && AppSeven.getInstance().getExternalCacheDir() != null) {
                sDefaultDir = AppSeven.getInstance().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            } else {
                sDefaultDir = AppSeven.getInstance().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
